...
export class EmployeeUpdateComponent implements OnInit {
  ...
  private loadEmployee(): void {
    this.graphqlService.getEmployeeById(this.employeeId).subscribe({
      next: (emp) => {
        this.currentPhoto = emp.employee_photo || '';
  ...
  onSubmit(): void {
    ...
    this.graphqlService.updateEmployee(this.employeeId, input, this.photoFile).subscribe({
      next: (result) => {
        this.loading = false;
        if (result.success === 'true') {
          ...
          this.snackBar.open(result.message || 'Update failed', 'Close', { duration: 4000 });
        }
      },
      error: (err) => {
        this.loading = false;
        this.snackBar.open(err.message || 'Update failed', 'Close', { duration: 4000 });
      }
    });
  }
